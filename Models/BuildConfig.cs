...
public class AppEntry
{
    ...
    /// <summary>
    /// Identificador do app no catálogo (nome do <c>AppId</c>, ex.: "Chrome"). No Windows os apps
    /// são instaladores baixados; no Linux este id mapeia para os pacotes da distro.
    /// </summary>
...
{
    // Sistema operacional de destino. Define qual pipeline gera a ISO (Windows ou Linux),
    // quais abas/opções aparecem e quais aplicativos ficam disponíveis.
    public TargetOs Os { get; set; } = TargetOs.Windows11;

    // Configurações específicas de Linux (ignoradas quando Os é Windows, mas preservadas
    // para o usuário alternar de sistema sem perder o que preencheu).
    public LinuxConfig Linux { get; set; } = new();

    /// <summary>Atalho: o sistema selecionado é uma distro Linux?</summary>
    [JsonIgnore] public bool IsLinux => OsCatalog.IsLinux(Os);
    /// <summary>Atalho: o sistema selecionado é Windows?</summary>
    [JsonIgnore] public bool IsWindows => OsCatalog.IsWindows(Os);
    ...
    public bool DebloatRemoveApps { get; set; }        // remove apps de fábrica (a lista escolhida abaixo)

    /// <summary>
    /// QUAIS apps de fábrica remover (IDs do <see cref="IsoForge.Core.BloatCatalog"/>).
    /// Vazia numa configuração salva por versão antiga: nesse caso vale o conjunto
    /// padrão, que é exatamente o que a opção única removia.
    /// </summary>
    public List<string> DebloatAppIds { get; set; } = new();
    ...
    // Tela CHEIA no primeiro logon em vez do terminal preto: icone do programa,
    // barra de progresso e tempo restante. E so apresentacao — quem instala continua
    // sendo o install.cmd, e a tela le um arquivo de status.
    public bool FullscreenProgress { get; set; } = true;
    ...
    public BuildConfig Clone()
    {
        ...
        c.Apps = new ObservableCollection<AppEntry>(Apps.Select(a => new AppEntry
        {
            ...
            CatalogId = a.CatalogId, RequiresInternet = a.RequiresInternet
        }));
    ...
    /// <summary>
    /// XML padrão do ODT mostrado na tela do Office.
    ///
    /// SEM &lt;RemoveMSI /&gt;, de propósito. Ele estava aqui e era o que fazia o Office
    /// offline falhar SEMPRE com 1603: com o RemoveMSI o bootstrapper roda o pré-requisito
    /// MSIxC2RCultureReachable, que exige os cabs da cultura en-us (i641033/s641033) — e
    /// eles não existem na fonte, porque o /download só baixa o idioma escolhido (pt-br,
    /// LCID 1046). Medido no Windows Sandbox sem rede: com a linha, 1603 e nada instalado;
    /// sem a linha, mesmo payload e mesmo setup.exe, o Office instalou da fonte local
    /// (MediaType = Local). Numa máquina recém-instalada por esta ISO não há Office MSI
    /// para remover, então a diretiva só trazia o pré-requisito. Ver OfficeConfig.
    /// (O OfficeConfig remove o RemoveMSI do XML de instalação de qualquer jeito — isto
    /// aqui é para ninguém copiar a linha de volta achando que ela é necessária.)
    /// </summary>
...
""";
    ...
    /// <summary>Gera o XML de configuração do ODT para o idioma escolhido (sem RemoveMSI — ver acima).</summary>
