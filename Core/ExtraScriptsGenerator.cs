...
public static class ExtraScriptsGenerator
{
    ...
    public static string WaitForInternet()
    {
        ...
        sb.AppendLine("  } catch {}");
        // NAO basta ter ROTA: o que o instalador precisa e RESOLVER NOME.
        //
        // Antes a reserva era um ping em '1.1.1.1' — um IP literal, que nao passa pelo
        // DNS. Numa maquina com rota boa e resolvedor morto isso gravava "internet OK" e,
        // logo depois, o ODT morria com 0x80072EE7 (ERROR_INTERNET_NAME_NOT_RESOLVED) sem
        // uma linha no log explicando. Foi exatamente o que aconteceu na validacao: o
        // Windows Sandbox subiu com o DNS do NAT sem responder, o teste disse OK e o
        // Office falhou sem motivo aparente. Custou uma rodada inteira para achar.
        //
        // Agora a reserva resolve o NOME de que o Office depende. Se o nome nao resolver,
        // o log diz isso com todas as letras em vez de deixar o proximo passo falhar em
        // codigo hexadecimal.
        sb.AppendLine("  try {");
        sb.AppendLine("    $ip = [System.Net.Dns]::GetHostAddresses('officecdn.microsoft.com')");
        sb.AppendLine("    if ($ip -and $ip.Count -gt 0) { return $true }");
        sb.AppendLine("  } catch {}");
        sb.AppendLine("  if (Test-Connection -ComputerName '1.1.1.1' -Count 1 -Quiet -ErrorAction SilentlyContinue) {");
        sb.AppendLine("    Write-Output 'IsoForge: ha rede, mas o DNS NAO resolve officecdn.microsoft.com. O Office online vai falhar com 0x80072EE7. Verifique o servidor DNS desta maquina.'");
        sb.AppendLine("  }");
        ...
        TaskbarAlignment taskbar = TaskbarAlignment.Default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(wallpaperFileName))
        {
            ...
            // -Type String NAO e detalhe. MEDIDO: o Windows guarda WallpaperStyle e
            // TileWallpaper como REG_SZ (String); o Set-ItemProperty -Value 10 grava
            // DWord. O Explorer le esses valores como TEXTO, entao com o tipo errado o
            // ajuste nao valia e a imagem caia no enquadramento padrao — era a causa de
            // "o papel de parede nao preenche a tela".
            //
            // 10 = Preencher: cobre a tela inteira cortando o excedente, sem distorcer.
            // 2 (Esticar) tambem preenche, mas deforma quando a proporcao da foto nao
            // bate com a do monitor, o que e a regra e nao a excecao.
            sb.AppendLine("  Set-ItemProperty 'HKCU:\\Control Panel\\Desktop' -Name WallpaperStyle -Value '10' -Type String -Force");
            sb.AppendLine("  Set-ItemProperty 'HKCU:\\Control Panel\\Desktop' -Name TileWallpaper -Value '0' -Type String -Force");
            ...
            sb.AppendLine("  }");
            // O enquadramento tambem no hive PADRAO. Trocar a img0.jpg faz todo usuario
            // novo ver a imagem certa, mas com o enquadramento que o Windows escolher —
            // metade do conserto valia so para quem ja tinha perfil.
            sb.AppendLine("  reg load HKU\\IsoForgeWp 'C:\\Users\\Default\\NTUSER.DAT' 2>$null | Out-Null");
            sb.AppendLine("  reg add 'HKU\\IsoForgeWp\\Control Panel\\Desktop' /v Wallpaper /t REG_SZ /d \"$wp\" /f 2>$null | Out-Null");
            sb.AppendLine("  reg add 'HKU\\IsoForgeWp\\Control Panel\\Desktop' /v WallpaperStyle /t REG_SZ /d 10 /f 2>$null | Out-Null");
            sb.AppendLine("  reg add 'HKU\\IsoForgeWp\\Control Panel\\Desktop' /v TileWallpaper /t REG_SZ /d 0 /f 2>$null | Out-Null");
            sb.AppendLine("  [gc]::Collect(); [gc]::WaitForPendingFinalizers()");
            sb.AppendLine("  reg unload HKU\\IsoForgeWp 2>$null | Out-Null");
