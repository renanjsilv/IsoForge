...
public static class SetupCompleteGenerator
{
    ...
    public static string SetupComplete(BuildConfig c)
    {
        ...
        sb.AppendLine(">>\"%LOGFILE%\" echo IsoForge (Entra ID / SetupComplete como SYSTEM) - inicio: %date% %time%");
        // %PROGRESSFILE% FALTAVA AQUI. O AppendAppInstalls abaixo e o MESMO do install.cmd e
        // emite linhas de status ('> "%PROGRESSFILE%" echo app^|1^|2^|...') sempre que
        // c.FullscreenProgress for true — e o padrao e true. Em batch, variavel indefinida
        // fica LITERAL: este script, que roda como SYSTEM com cwd em C:\Windows\System32,
        // criava um arquivo chamado "%PROGRESSFILE%" no System32. Nao impedia a instalacao,
        // mas mostra que este caminho (modo Entra ID) nunca tinha sido executado de verdade
        // — e e por ele que o Office passa quando a ISO e gerada nesse modo.
        sb.AppendLine($"set \"PROGRESSFILE={InstallScriptGenerator.ProgressFileVar}\"");
        ...
        sb.AppendLine(">>\"%LOGFILE%\" echo IsoForge (Entra ID) - fim: %date% %time%");
