...
public static class UnattendGenerator
{
    ...
    static XElement? SettingsSpecialize(BuildConfig c)
    {
        ...
        // Na seleção de unidade o nome é definido pelo operador, não aqui.
        if (!c.UseUnitSelection && !string.IsNullOrWhiteSpace(c.ComputerName))
            itens.Add(Component("Microsoft-Windows-Shell-Setup",
                new XElement(U + "ComputerName", c.ComputerName.Trim())));

        // O EXPLORER NÃO SOBE NO 1º LOGON.
        //
        // Numa máquina recém-formatada o Windows abre o menu Iniciar sozinho, e ele
        // vem POR CIMA da tela cheia — a escolha de unidade e o progresso ficavam
        // atrás. Não há chave suportada para "não abrir o Iniciar"; trocar o shell
        // resolve na raiz, porque sem Explorer não há Iniciar, nem barra de tarefas,
        // nem área de trabalho para disputar a tela.
        //
        // Aqui (specialize) porque é o último passe que roda ANTES do primeiro logon,
        // e como SYSTEM — escrever em HKLM\...\Winlogon exige isso. Quem desfaz é o
        // install.cmd, logo no início, onde ainda há privilégio.
        if (c.FullscreenProgress && c.Mode == DeploymentMode.LocalAccount)
        {
            var run = new XElement(U + "RunSynchronous",
                new XElement(U + "RunSynchronousCommand",
                    ...
                    new XElement(U + "Path",
                        ...
                        $@"/v Shell /t REG_SZ /d ""wscript.exe //B //Nologo C:\Setup\{InstallScriptGenerator.ShellStubFileName}"" /f"),
                    new XElement(U + "Description", "IsoForge - shell do 1o logon (evita o menu Iniciar por cima da tela)")));
            itens.Add(Component("Microsoft-Windows-Deployment", run));
        }

        if (itens.Count == 0) return null;
        return new XElement(U + "settings", new XAttribute("pass", "specialize"), itens);
    }
    ...
    static XElement SettingsOobe(BuildConfig c)
    {
        ...
        shell.Add(new XElement(U + "FirstLogonCommands",
            new XElement(U + "SynchronousCommand",
                new XAttribute(Wcm + "action", "add"),
                new XElement(U + "Order", 1),
                // OCULTO quando ha tela cheia; visivel quando nao ha.
                //
                // O FirstLogonCommands abre um console, e ele ficava ATRAS da tela
                // cheia de progresso: um retangulo preto nas bordas e no alt-tab,
                // estragando justamente o que a tela existe para dar. Esconder so faz
                // sentido QUANDO ha a tela — sem ela o console e o unico retorno
                // visivel, e ocultar deixaria a maquina parecendo parada.
                new XElement(U + "CommandLine", c.FullscreenProgress
                    ? $@"wscript.exe //B //Nologo C:\Setup\{InstallScriptGenerator.LauncherFileName}"
                    : @"cmd.exe /c C:\Setup\install.cmd"),
