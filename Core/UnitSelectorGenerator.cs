...
public static class UnitSelectorGenerator
{
    ...
    /// <summary>
    /// O array PowerShell com as unidades configuradas. Compartilhado com a tela única
    /// do 1º logon (<see cref="ProgressUiGenerator"/>) — as duas telas mostram a mesma
    /// lista, e duplicar a montagem garantiria que um dia divergissem.
    /// </summary>
    internal static string ArrayUnidades(BuildConfig cfg)
    {
    ...
    /// <summary>
    /// As REGRAS DO NOME NetBIOS, em um lugar só: prefixo + nº de série do BIOS, sem
    /// caractere inválido, no máximo 15, sem hífen nas pontas. Compartilhada para a
    /// tela nova não reimplementar (e errar) as mesmas regras.
    /// </summary>
    internal const string FuncaoGetHostname = """
...
""";
    ...
    /// <summary>
    /// Renomeia a máquina, com a queda para CIM que o Rename-Computer às vezes exige.
    /// Compartilhada pelo mesmo motivo das anteriores.
    /// </summary>
    internal const string FuncaoRenomear = """
function Renomear([string]$novo) {
    try {
        Rename-Computer -NewName $novo -Force -ErrorAction Stop
        return "renomeado para $novo (aplica no reinicio final)"
    } catch {
        try {
            $rc = Invoke-CimMethod -ClassName Win32_ComputerSystem -MethodName Rename -Arguments @{ Name = $novo }
            return "Rename-Computer falhou; CIM devolveu $($rc.ReturnValue)"
        } catch {
            return "rename falhou: $($_.Exception.Message)"
        }
    }
}
""";
    ...
    {
        var units = new StringBuilder(ArrayUnidades(cfg));
...
# O @( ) externo garante ARRAY: "@(...) | Where-Object" devolve objeto solto quando
# sobra um item so, e .Count num PSCustomObject e $null (ver ProgressUiGenerator).
$Unidades = @(@(
{{units}}    $null
) | Where-Object { $_ })
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        ResizeMode="NoResize" Background="#0B1220" FontFamily="Segoe UI Variable, Segoe UI">
  <Grid>
{{FullscreenChrome.Gatilho(comAnel: false)}}
    <Grid.RowDefinitions>
